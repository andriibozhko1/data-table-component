export default class TableList {
  ...
  _render(id = 0, quantityPhones = 5, phonesList = this.phones) {
    ...
    <table class="table__table">
          ...
          <td data-check-all="false"><input type="checkbox" ${
            this.generalStatusCheckbox ? "checked" : ""
          }></input</td>
          ${Object.keys(this.columnConfig)
            .map(typeOfSort => {
              ...
              <th class="table__type-of-sort" 
                ...
                data-is-searchable="${
                  this.columnConfig[typeOfSort].isSearchable
                }"
                data-sort-to="up"
                >
          ...
          pagination[id] !== undefined
            ? pagination[id]
                .map(phone => {
                  return `
                  <tr>
                    <td data-item-id="${phone.id}" data-check-item="${
                    phone.isChecked
                  }"><input type="checkbox" ${
                    phone.isChecked ? "checked" : ""
                      ...
                      ${Object.keys(this.columnConfig)
                        .map(value => {
                          return `
                            <td class="table__type-of-sort"  data-items-id="${
                              phone.id
                            }">
                              <div class="table__data-field" data-items-type="${value}" data-items-value="${
                            phone[value]
                          }">
                                ${phone[value]}
                              </div>
    ...
    this.element.addEventListener("click", event => {
      ...
      if (event.target.closest("[data-check-item]")) {
        if (event.target.checked) {
          this.phones.find(phone =>
            phone.id ===
            event.target.closest("[data-check-item]").dataset.itemId
